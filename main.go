...
func GetDetails(req *http.Request, resp *http.Response) string { // Added GetDetails
	...
	if req.URL.Scheme == "https" {
		...
		cn = append(cn, strings.ToLower(resp.TLS.PeerCertificates[0].Subject.CommonName))
		cn = append(cn, resp.TLS.PeerCertificates[0].DNSNames...)
		unique.Strings(&cn)
	}
	return fmt.Sprintf("[%d] [%s] [%s] [%s] [%s]", status, size, title, server, strings.Join(cn, ","))
}
